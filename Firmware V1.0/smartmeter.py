### VERSION 0003 ***DO NOT REMOVE THIS LINE***
...
def send_data_as_message(data,uart=uart0):
    ...
    sendCMD_waitResp(f'AT+CMGS="{phone}"\r\n')
    utime.sleep(2)
...
def fixDateSettings():
    sendCMD_waitResp('AT+CTZU=1') # Automatic timezone reporting 
    sendCMD_waitResp('AT+CTZR=1') # most operator these days support Network Identity and Time Zone Updates (old ones are: 
    sendCMD_waitResp('AT&W') # write to nvram of gsm module (effective after reboot)
    print('AFTER THIS PLEASE REBOOT THE GSM MODULE AND THEN CHECK DATE AND TIME')
